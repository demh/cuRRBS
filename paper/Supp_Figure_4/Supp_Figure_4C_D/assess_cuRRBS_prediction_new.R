...
#####                                 cuRRBS paper                                     ####
###########################################################################################
##### Assess how well cuRRBS performs when comparing it with RRBS experimental data    ####
###########################################################################################
##### USAGE: manual                                                                    ####
###########################################################################################
...
setwd('~/Desktop/methylation_clock/optimize_RRBS/cuRRBS_paper/Figure_4/Figure_4new/mouse_adipogenesis/');
...
sites_annotation <- read.table('CG_sites_mouse/CpG_in_mouse_CGI_annotation.csv', sep=',', header=T);
cuRRBS_output <- readLines('final_cuRRBS_output_80_160_rl75.csv');
bismark_output <- read.table('edit_Lim_2016_cov_files/SRR3467843_BWAT_RRBS_D6_1_val_1_GRCm38_bismark_bt2_pe.bismark.cov_mod', sep='\t', header=F); # We have deleted the sites in chrM
...
ggsave('positives_negatives_barplots_80_160_rl75_SRR3467843_BWAT.pdf', width=8, height=8);
...
ggsave('sensitivity_specificity_80_160_rl75_SRR3467843_BWAT.pdf', width=8, height=8);
