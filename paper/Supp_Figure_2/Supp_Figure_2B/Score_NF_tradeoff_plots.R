...
##### This information will be used to make Fig2B, Fig2C and SuppFig2B.                ####
###########################################################################################
...
option_list <-  list(
  ...
  make_option(c('-f', '--fld_file'), type='character', default=NULL, 
              help="Absolute path to the fl_distribution_x_y.txt file, which is created by \
                obtain_distributions_and_fragments.py. It contains information for one enzyme (combination). COMPULSORY",
              metavar='character'),
  ...
  make_option(c('-s', '--window_step'), type='integer', default=10, 
              help="Step (in bp) taken every time the window slides. DEFAULT=10",
              metavar='integer'),
  
  make_option(c('-w', '--window_width_step'), type='integer', default=10, 
...
#output_path <- '~/Documents/Manuscripts/cuRRBS/Figures/Figure_2/2B_2C/';
setwd(output_path);
...
NF_1000_thr <- 1343.468; # NF/1000 threshold (it can be changed)


# Input files.

fsd_file_path <- as.character(opt$fld_file);
#fsd_file_path <- '~/Documents/Manuscripts/cuRRBS/Figures/Figure_2/2B_2C/fl_distributions_1_enzymes.txt';
si_file_path <- as.character(opt$fragments_file);
#si_file_path <- '~/Documents/Manuscripts/cuRRBS/Figures/Figure_2/2B_2C/fragments_of_interest_1_enzymes.txt';
sites_annotation <- read.table(as.character(opt$annotation_file), sep=',', header=T);
#sites_annotation <- read.table('~/Documents/Manuscripts/cuRRBS/Figures/Figure_2/2B_2C/human_clock_sites_annotation.csv', sep=',', header=T);
...
arrow_data <- data.frame(x=optimal_NF-100, y=optimal_Score+7, xend=optimal_NF-7.5, yend=optimal_Score+0.5);
...
if(create_plot){
  ...
  plot_tradeoff <- ggplot(final_data, aes(x=NF_1000, y=final_perc_max_Score)) + 
    ...
    annotate("text", x=optimal_NF-150, y=optimal_Score+10, label= paste0(gsub('_', '-', optimal_size_range), ' bp'),
             col='darkorange');
